...
create table if not exists usuarioscursos(usuario_id int, curso_id int, completed boolean,primary key(usuario_id, curso_id), index u_id (usuario_id), index c_id (curso_id), foreign key (usuario_id) references usuarios(id) on update cascade on delete cascade, foreign key (curso_id) references cursos(id) on update cascade on delete cascade) ENGINE=INNODB;
